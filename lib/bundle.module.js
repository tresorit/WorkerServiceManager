...
(function (PortCommands) {
    ...
    PortCommands[PortCommands["resolve"] = 2] = "resolve";
    PortCommands[PortCommands["reject"] = 3] = "reject";
})(PortCommands$1 || (PortCommands$1 = {}));
class DeferredPromise$1 {
    constructor() {
        this.promise = new Promise((res, rej) => {
            this.resolve = res;
            this.reject = rej;
...
class PortHandler$1 {
    ...
    async call(service, method, args) {
        const deferred = new DeferredPromise$1();
        const pid = this.nextPid++;
        this.deferreds.set(pid, deferred);
        this.port.postMessage([PortCommands$1.call, pid, service, method, args]);
        return deferred.promise;
    }
    setCallHandler(handler) {
        if (this.callHandler !== null)
    ...
    fire(service, method, args) {
        this.port.postMessage([PortCommands$1.fire, service, method, args]);
    }
    async handleMessage(ev) {
        switch (ev.data[0]) {
            case PortCommands$1.fire:
                try {
                    // We intentionally ignore the result (and even the errors) of the call. This is rarely a good idea,
                    // but it is sometimes useful.
                    //noinspection JSIgnoredPromiseFromCall
                ...
                break;
            case PortCommands$1.call:
                try {
                    const res = await this.callHandler(ev.data[2], ev.data[3], ev.data[4]);
                    this.port.postMessage([PortCommands$1.resolve, ev.data[1], res]);
                }
                catch (ex) {
                    this.port.postMessage([PortCommands$1.reject, ev.data[1], ex]);
                }
                break;
            case PortCommands$1.resolve:
                this.deferreds.get(ev.data[1]).resolve(ev.data[2]);
                break;
            case PortCommands$1.reject:
                this.deferreds.get(ev.data[1]).reject(ev.data[2]);
                break;
        }
    }
...
=======
                ...
                const proxy = new serviceProxyInfo[1](portHandler);
>>>>>>> 112360d10fe627892234104a48a20bbcf4fb0c5f
...
export { WorkerServiceManager, RemoteService, PortHandler$1 as PortHandler };
